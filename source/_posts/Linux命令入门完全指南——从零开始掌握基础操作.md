...
```
$ wc -l ./data/messages
wc: ./data/messages: Permission denied
...
**原因**：系统日志文件 `/var/log/messages` 权限为 `600`（-rw-------），只有 root 用户可以读取。复制后的文件保留了原权限。
...
- 每次操作都加 `sudo`
  ...
  ```
  sudo wc -l ./data/messages
  sudo grep -i "error" ./data/messages | wc -l
  ```
...
```
$ ./analyze.sh
./analyze.sh: line 16: ./data/messages: Permission denied
[sudo] password for g6017: 
sudo: a password is required
```



**原因**：脚本中部分命令用了 sudo，部分没用，导致权限不一致。
...
**原因**：`tail -f` 会持续监听文件新内容，不是卡住，是在等待新日志。
...
- 如果只想分析现有日志，去掉 `-f`
  ...
  ```
  sudo grep -i "error" /var/log/messages > ./data/error-stream.log
  ```
...
```
$ tar -czf backup.tar.gz ~/log-analysis/
tar: /home/g6017/log-analysis/data/messages: Cannot open: Permission denied
```



**原因**：`messages` 文件只有 root 可读，普通用户无法读取。
...
- 用 sudo 备份
  ...
  ```
  sudo tar -czf backup.tar.gz /home/g6017/log-analysis/
  sudo chown g6017:g6017 backup.tar.gz
  ```
...
- 或排除该文件
  ...
  ```
  tar -czf backup.tar.gz --exclude='data/messages' ~/log-analysis/
  ```
...
```
$ find /tmp -name "*.tar.gz" -mtime +7 -delete
find: ‘/tmp/systemd-private-xxx’: Permission denied
find: ‘/tmp/vmware-root_xxx’: Permission denied
...
```



**原因**：`/tmp` 目录中有很多属于其他服务的私有目录，普通用户无权限访问。
...
- 或用 sudo 运行
  ...
  ```
  sudo find /tmp -name "*.tar.gz" -mtime +7 -delete
  ```
...
**现象**：在 vim 中按 `Esc` 没用，不知道如何保存退出。

**解决方法**：

- 强制退出不保存：`Esc` → `:q!` → 回车
- 保存退出：`Esc` → `:wq` → 回车
- 如果完全卡住：按 `Ctrl+C` 或 `Ctrl+Z` 中断
...
```
报告已生成: /home/g6017/log-analysis/../reports/report-20260301.txt
$ cat ./reports/report-*.txt
cat: './reports/report-*.txt': No such file or directory
```



**原因**：路径中的 `..` 表示上一级目录，实际文件在 `/home/g6017/reports/` 而不是 `/home/g6017/log-analysis/reports/`。
...
```
$ sudo cat ./data/messages | grep -i "error" | wc -l
0
```



**原因**：虽然 `cat` 用 sudo 执行，但管道后的 `grep` 和 `wc` 是用普通用户权限执行的。如果 `cat` 失败（权限不足），管道后面接收不到数据。
...
- 或使用 `sudo bash -c`
  ...
  ```
  sudo bash -c "cat ./data/messages | grep -i 'error' | wc -l"
  ```
...
1. **权限是 Linux 的核心概念**——遇到 "Permission denied" 首先想到用 `sudo` 或检查文件权限
2. **sudo 只对当前命令有效**——每个需要特权的命令都要加 sudo，除非切换到 root 用户
3. **路径要仔细**——`.` 是当前目录，`..` 是上一级目录，容易混淆
4. **管道命令要注意**——sudo 只在它所在的命令段有效
5. **tail -f 不会自动退出**——它是实时监控命令，需要手动停止
6. **vim 有模式概念**——分清命令模式、插入模式和末行模式

这些"坑"其实是学习 Linux 的必经之路，每遇到一次，理解就更深一层。希望这份问题整理能帮助后来的学习者少走弯路。
